...
| `$pill` | `#eef1f8` | pill·아이콘 배경, **표면 위 목록 행 hover** |
...
| 대상 | hover 배경 |
|---|---|
| 흰 카드(`$surface`) 안을 꽉 채우는 **목록 행**·전폭 버튼 | `$pill` |
| 테두리가 있는 Ghost 버튼·아이콘 버튼 | `$page-bg` |
| 네이비 버튼 / Danger 버튼 | `$navy-hover` / `$red-hover` |

> **행 hover 에 `$page-bg` 를 쓰지 않습니다.** 색이 페이지 배경과 같아서, 카드에 구멍이 뚫려
> 페이지 바닥이 보이는 것처럼 읽힙니다. 테두리로 이미 경계가 잡힌 버튼은 이 착시가 없어
> `$page-bg` 를 그대로 씁니다. 반대로 푸터색(`$footer-compact-bg` #f6f7fa)은 흰색에 더 가까워
> hover 가 흐려지므로 hover 로 쓰지 않습니다.
