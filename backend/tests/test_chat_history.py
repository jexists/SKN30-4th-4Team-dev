...
def test_add_message_bumps_room_to_top(history_client):
    ...
    # 먼저 만든 방에 메시지를 넣으면 updated_at 이 갱신돼 목록 맨 위로 올라와야 한다.
...
def test_cross_user_room_messages_404(history_client):
    ...
    assert resp.json()["error"]["title"] == "NOT_FOUND"
...
def test_history_unavailable_returns_503(monkeypatch):
    """저장소 미설정(AppSessionLocal None)이면 503(HISTORY_UNAVAILABLE)."""
    ...
    try:
        ...
        assert resp.json()["error"]["title"] == "HISTORY_UNAVAILABLE"
    finally:
        app.dependency_overrides.clear()
