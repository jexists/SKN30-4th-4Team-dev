...
RunPod GPU 배포 절차는 [`../docs/runpod-ocr.md`](../docs/runpod-ocr.md)를 참고하세요.
...
- 페이지별 레이아웃 Parsing(분석 텍스트)과 Spotting(마스킹 좌표) 분리
- PDF/PNG 공통 여백 제거·대비 보정·최대 크기 정규화
- 주민등록번호·전화번호·이메일·계좌번호 규칙 탐지
- 문맥 라벨이 있는 이름 탐지(주소는 문서 간 일치 분석을 위해 유지)
- 픽셀 기반 마스킹 및 평탄화 PDF 생성
- LLM 전달용 텍스트의 개인정보 플레이스홀더 치환
- 마스킹 결과 재-OCR 검증
- 입력/중간 파일은 요청별 임시 디렉터리에만 저장

Spotting의 글자·단어별 좌표 스키마는 실제 v1.6 한국어 계약서 PoC로 계속 검증해야
합니다. 현재는 개인정보가 포함된 블록 전체를 가리고 결과를
`X-Review-Required: true`로 표시하는 보수적 fallback을 사용합니다.

AI 분석용 본문은 Spotting 문자열을 이어 붙이지 않고 레이아웃 Parsing 결과의
`block_order`를 우선해 생성합니다. 레이아웃 결과가 비었을 때만 Spotting 텍스트로
폴백합니다. `OCR_USE_LAYOUT_DETECTION=true`의 최초 실행에는 공식 레이아웃 모델 다운로드가
추가될 수 있으므로 RunPod 네트워크 볼륨의 모델 캐시를 유지하는 것을 권장합니다.
...
않습니다. 다만 한 요청 안에서 문서 Parsing, 마스킹 좌표 Spotting, 마스킹 잔존 검증은
목적이 달라 각각 실행합니다. 분석용 텍스트는 탐지된 값을 `[이름]`, `[전화번호]` 같은
플레이스홀더로 치환한 뒤 다시 검사합니다. 지원하는 개인정보 패턴이 남아 있으면
응답하지 않고 422로 중단합니다.

## PDF 입력 정규화

PDF는 `OCR_RENDER_DPI`로 페이지별 PNG를 만든 뒤 PNG/JPEG 입력과 같은 전처리를 거칩니다.
큰 흰 여백은 `OCR_AUTO_CROP`, 지나치게 큰 페이지는 `OCR_MAX_IMAGE_LONG_EDGE`로 제한합니다.
페이지별 원본·crop·최종 크기와 Parsing/Spotting 영역 수는 원문 없이 로그에 남습니다.
정확도 문제가 있을 때 DPI만 올리기 전에 이 로그로 실제 문서 영역 크기를 확인해야 합니다.

이름 탐지는 현재 `임대인:`, `임차인:`, `소유자:`, `성명:` 같은 문맥 라벨을
사용하며, 표에서 라벨과 값이 분리된 경우 바로 앞 OCR 영역의 문맥도 확인합니다.
주소는 등기부등본·계약서·건축물대장의 일치 여부 분석을 위해 마스킹하지 않고 분석
모델에 전달합니다. 모든 자유 형식 개인정보를 보장하는 NER 모델은 아니므로
